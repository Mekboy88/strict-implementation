...
function UrDevEditorPage() {
  ...
  const [activeActivityIcon, setActiveActivityIcon] = useState<'folder' | 'code' | 'run' | 'database'>('folder');
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <aside className="hidden lg:flex w-12 flex-col items-center justify-between border-r border-white/10 bg-[#030711] py-3">
          <div className="flex flex-col items-center gap-3">
            <button
              type="button"
              onClick={() => setActiveActivityIcon('folder')}
              className={`transition-colors ${activeActivityIcon === 'folder' ? 'text-sky-400' : 'text-slate-400 hover:text-slate-200'}`}
              title="Explorer"
            >
              <svg viewBox="0 0 36 36" className="h-4 w-4" fill="currentColor">
                <path fill={activeActivityIcon === 'folder' ? 'currentColor' : '#269'} d="M0 29a4 4 0 0 0 4 4h24a4 4 0 0 0 4-4V12a4 4 0 0 0-4-4h-9c-3.562 0-3-5-8.438-5H4a4 4 0 0 0-4 4v22z"></path>
                <path fill={activeActivityIcon === 'folder' ? 'currentColor' : '#55ACEE'} d="M30 10h-6.562C18 10 18.562 15 15 15H6a4 4 0 0 0-4 4v10a1 1 0 1 1-2 0a4 4 0 0 0 4 4h26a4 4 0 0 0 4-4V14a4 4 0 0 0-4-4z"></path>
              </svg>
            </button>
            <button
              type="button"
              onClick={() => setActiveActivityIcon('code')}
              className={`inline-flex h-8 w-8 items-center justify-center rounded-xl transition-colors ${activeActivityIcon === 'code' ? 'bg-sky-500/20 text-sky-400' : 'bg-white/5 text-slate-200 hover:bg-white/10'}`}
              title="Code"
            >
            ...
            <button
              type="button"
              onClick={() => setActiveActivityIcon('run')}
              className={`inline-flex h-8 w-8 items-center justify-center rounded-xl transition-colors ${activeActivityIcon === 'run' ? 'bg-sky-500/20 text-sky-400' : 'bg-white/5 text-slate-200 hover:bg-white/10'}`}
              title="Run"
            >
            ...
            <button
              type="button"
              onClick={() => setActiveActivityIcon('database')}
              className={`transition-colors ${activeActivityIcon === 'database' ? 'text-sky-400' : 'text-slate-400 hover:text-slate-200'}`}
              title="Database"
            >
